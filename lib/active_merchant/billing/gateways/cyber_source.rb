  ...
  module Billing #:nodoc:
    ...
    class CyberSourceGateway < Gateway
      ...
      def build_auth_request(money, creditcard_or_reference, options)
        ...
        add_payment_network_token(xml, creditcard_or_reference)
      ...
      def build_purchase_request(money, payment_method_or_reference, options)
        ...
        if (!payment_method_or_reference.is_a?(String) && card_brand(payment_method_or_reference) == 'check') || reference_is_a_check?(payment_method_or_reference)
          ...
          add_airline_data(xml, options)
          add_sales_slip_number(xml, options)
          add_payment_network_token(xml, payment_method_or_reference)
      ...
      def build_create_subscription_request(payment_method, options)
        ...
        if options[:setup_fee]
          if card_brand(payment_method) == 'check'
            ...
            add_payment_network_token(xml, payment_method)
          end
        end
      ...
      def add_payment_network_token(xml, payment_method)
        return unless network_tokenization?(payment_method)

        transction_type = payment_method.source == :network_token ? '3' : '1'
        xml.tag! 'paymentNetworkToken' do
          xml.tag!('transactionType', transction_type)
        end
      end
